...
- Add a `dump` subcommand (Python: `ClickhouseCluster.dump()`) that prints every table, view, materialized view and dictionary of a database as portable, dependency-ordered SQL: no `UUID`, no database prefix, `Replicated*MergeTree` path/replica arguments removed (`--keep-replicated-paths` keeps them), the migrations and lock tables and `.inner` tables excluded. `--out FILE` writes it atomically, `--check FILE` exits 1 with a unified diff on drift for CI, `--tables` limits the objects. Read-only; works with both drivers. Done by @zifter in https://github.com/zifter/clickhouse-migrations/pull/PRNUM. Part of #83.
