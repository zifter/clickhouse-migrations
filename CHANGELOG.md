...
- Add `status --strict` (exit code 1 on `md5-mismatch` / `unknown` rows), `--exit-code-pending` (exit code 1 on `pending` rows), `--format json` for machine-readable output and a `has_down` column showing whether a `.down.sql` exists. Done by @zifter in PR_URL. Closes #77.
