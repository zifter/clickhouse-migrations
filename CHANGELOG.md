...
- Add an offline `validate` subcommand (Python: `validate_migrations()`) that checks a migrations directory without any database. Errors (exit code 1): bad file names, duplicate versions, orphan `.down.sql`, empty files, comment-only statements (which ClickHouse rejects as an empty query), unterminated strings / quoted identifiers / block comments, non-UTF-8 files. Warnings (exit code 1 with `--strict`): version gaps, destructive statements (`DROP TABLE`, `TRUNCATE`, `ALTER … DELETE/UPDATE`, `DROP COLUMN`, …; never for `.down.sql`), missing `.down.sql` (`--require-down` makes it an error), a standalone `SET` in a multi-statement file, inconsistent `ON CLUSTER`. It reuses the `migrate` tokenizer, so strings and comments never trigger a check; `--format json` for machines. Ships a `.pre-commit-hooks.yaml` with the `clickhouse-migrations-validate` hook. `migrate` is unchanged. Done by @zifter in https://github.com/zifter/clickhouse-migrations/pull/PR_NUMBER. Closes #84.
