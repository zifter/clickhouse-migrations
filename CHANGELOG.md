...
- Add `baseline` and `repair` subcommands (Python: `ClickhouseCluster.baseline()` / `.repair()`). `baseline --to VERSION` adopts an existing database: it records every local migration up to `VERSION` as applied (md5 and script, like `--fake`) without executing anything, refuses to run unless the migrations table is absent or empty, and `--dry-run` prints the resulting status and writes nothing. `repair` reports applied migrations whose file changed (`md5-mismatch`) or disappeared (`unknown`) and exits 1 if there are any; `--write` updates the stored md5 and script of exactly those rows (insert, then a synchronous `ALTER TABLE … DELETE … SETTINGS mutations_sync = 2` that waits for every replica), `--version N` (repeatable) narrows it, and `--prune --write` deletes `unknown` rows. Both print the `status` table (or `--format json`) and honour `--lock` for real writes. Done by @zifter in PR_URL. Closes #82.
